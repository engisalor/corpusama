...
# this script tries to guess the actual gender and number
# of Spanish noun-phrase constituents by looking at their context
# expects a vertical file tagged by FreeLing tagger with
# the following order of columns: word, tag, [..]
# the guessed gender and number are output as new columns
# at the end of each token line
...
# regular expressions for recognized token classes, each containing a:
# - class code, as:  capture group P<class>, or 3rd character from the
#   end in the last group's name
# - gender, as:  capture group P<gen>, or 2nd character from the end in
#   the last group's name
# - number, as:  capture group P<num>, or 1st character from the end in
#   the last group's name
TOKEN_CLASS_RE_DICT = {
    ...
    \t  # tab
    (?P<class>S)P  # tag
    (?P<x_00>)  # hardcoded values
    """,
        re.X,
    ),
    "pronoun_1": re.compile(
        r"""^  # start
    [^\t]*  # word
    \t  # tab
    (?P<class>P)[DI][^\t](?P<gen>[^\t])(?P<num>[^\t])  # tag
    """,
        re.X,
    ),
    "pronoun_2": re.compile(
        r"""^  # start
    (?i:cuyo|cuya|cuyos|cuyas)  # word
    \t  # tab
    (?P<class>P)R[^\t](?P<gen>[^\t])(?P<num>[^\t])  # tag
    """,
        re.X,
    ),
    "pronoun_3": re.compile(
        r"""^  # start
    (?i:cuántos|cuántas)  # word
    \t  # tab
    (?P<class>P)T[^\t](?P<gen>[^\t])(?P<num>[^\t])  # tag
    """,
        re.X,
    ),
...
# uses token classes
PHRASE_RE = re.compile("(D|S|P|DD|SD|DP|SP|DDD|SDD|DDP|SDP)?[ANVO]{0,9}[ANV]")

DEFINITE_GEN_RE = re.compile(r"[MF]")
DEFINITE_NUM_RE = re.compile(r"[SP]")


# split iterator into lists of elements by separator (included as list's last
# element), limiting maximal list size
